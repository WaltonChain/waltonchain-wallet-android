...
class ViewHolderTransaction(override val containerView: View, private val l: ClickListener) : ViewHolder(containerView), LayoutContainer {
    ...
    fun bind(transactionRecord: TransactionViewItem, showBottomShade: Boolean) {
        ...
        if (transactionRecord.status is TransactionStatus.Pending && !incoming && transactionRecord.coinValue.coin.type == CoinType.Ethereum) {
            if (transactionRecord.currencyValue == null) {
                txValueInFiat.text = ""
            } else {
                val cv = transactionRecord.currencyValue.copy(currency = transactionRecord.currencyValue.currency, value = transactionRecord.currencyValue.value.movePointLeft(9))
                txValueInFiat.text = App.numberFormatter.formatForTransactions(cv, incoming, canUseLessSymbol = true, trimmable = false)
            }
            txValueInCoin.text = App.numberFormatter.format(CoinValue(transactionRecord.coinValue.coin, transactionRecord.coinValue.value.movePointLeft(9)), explicitSign = true, realNumber = true)
        } else {
            if (transactionRecord.currencyValue == null) {
                txValueInFiat.text = ""
            } else {
                txValueInFiat.text = App.numberFormatter.formatForTransactions(transactionRecord.currencyValue, incoming, canUseLessSymbol = true, trimmable = false)
            }
            txValueInCoin.text = App.numberFormatter.format(transactionRecord.coinValue, explicitSign = true, realNumber = true)
        }
