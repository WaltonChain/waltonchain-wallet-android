...
class EthereumKitManager(appConfig: IAppConfigTestMode) : IEthereumKitManager {
    ...
    override fun ethereumKit(wallet: Wallet): EthereumKit {
        ...
        if (accountType is AccountType.Mnemonic && accountType.words.size == 12) {
            ...
            return kit!!
        } else if (accountType is AccountType.PrivateKey && accountType.key.isNotEmpty()) {
            useCount += 1
            ...
            kit = EthereumKit.getInstance(App.instance, accountType.key.toBigInteger(), syncMode, networkType, infuraCredentials, etherscanKey, account.id)
            kit?.start()
