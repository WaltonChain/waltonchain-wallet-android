...
class EthereumAdapter(kit: EthereumKit) : EthereumBaseAdapter(kit, decimal) {
    ...
    private fun transactionRecord(transaction: TransactionInfo): TransactionRecord {
        ...
        val fee = transaction.gasUsed?.toBigDecimal()?.multiply(transaction.gasPrice.toBigDecimal())?.movePointLeft(decimal / 2)
        ...
        return TransactionRecord(
                ...
                amount = transaction.value.toBigDecimal().movePointLeft(decimal / 2),
                fee = fee,
